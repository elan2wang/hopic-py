...
class SlotLayout:
    ...
    def __init__(self, time_steps):
        # base, height
        self.layout = [[None, None] for i in range(time_steps)]
        return

    def setItem(self, idx, base, height):
        self.layout[idx] = [base, height]
        return
...
class SlotSegment:
    ...
    def setInteractionSession(self, interaction_session, session_layout):
        ...
        if self.starting_time == (
                None or self.starting_time > interaction_session.start_time):
...
class SessionLayout:
    ...
    def __init__(self, slot_number, interaction_session):
        ...
        self.emerging_lines = {'will_rise': Set(),
                                                     'will_drop': Set(),
                                                     'will_die': Set()}
        return
...
class TubeCluster:
    ...
    def __init__(self, start_time, tube_idx):
        self.start_time = start_time
    ...
    def addMinMax(self, minimum, maximum):
        self.min_pos.append(minimum)
        self.max_pos.append(maximum)
        return
...
def decodeGenome(genome, interaction_sessions):
    slot_base_layout = SlotBaseLayout(SLOTS)
...
def generateSlotSegments(slot_base_layout):
    ...
    slot_segments = SlotSegments(SLOTS)
...
def generateLayout(genome, data):
    ...
    slot_layouts = [SlotLayout(data['time_step']) for i in range(SLOTS)]
...
def loadData(iflnm):
    ...
    for character_name in character_index_dict:
        ...
        character = Character(character_name, character_id)
...
def outputLayout(oflnm, layout, data):
    ...
    for member in layout:
        ...
        ofl.write(characters[member].name)
        for time_step in range(time_steps):
            ofl.write('\t')
